...
#
# Discussion Log: Project 2
# Used the following sources to gain a better understanding of Python and HTML:
# https://www.geeksforgeeks.org/check-if-string-contains-substring-in-python/
# https://www.shecodes.io/athena/5593-change-color-of-an-entire-html-webpage#:~:text=To%20change%20the%20color%20of,property%20to%20the%20desired%20color.
...
#
# Discussion Log: Project 3
# Used the following sources to gain a better understanding of Python:
...
topic_list_version = 0
topics = []
updates = threading.Condition()

class Topic:
    def __init__(self, name, msg_count =0, like_count =0):
...
def handle_http_get_hello(req, conn):
    ...
    if params:
        ...
        if '+' in name:
            name = name.replace('+', ' ')
        color = inputs[1]
        favcolor = color.split('=')
        favcolor = favcolor[1]
    msg = f'<html><head>Welcome, {name}!!</head>'
    msg+= '<title>Hello</title>'
    msg += f'<body style = "background-color: {favcolor};">'
    msg += '<p>The current date is %s.</p>' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += '<p>You have visited this page %d times before.</p>' % count
    msg += f'<p>Your favorite color is... {favcolor}!</p>'
    msg += '<p>Hit page refresh (F5) to refresh this page and see the page count update</p>'
    msg += '<p>You can also go to these exciting pages:</p>'
    msg += f'<a href =  "http://{server_host}:{server_port}/hello"> - this page </a>'
    msg += f'<a href =  "http://{server_host}:{server_port}/status"> - status and statistics </a>'
    msg += f'<a href = "http://{server_host}:{server_port}/quote"> - a random quote </a>'
    msg += f'<a href = "http://{server_host}:{server_port}/welcome.html"> - a welcome page </a>'
    msg += f'<a href = "http://{server_host}:{server_port}/index.html"> - some fancy web page </a>'
    msg += "\n"
    msg+= '</body> </html>'
...
def handle_http_get_file(url_path):
    ...
    try:
        ...
        css_type = ".css"
        js_type = ".js"
        if htm_type in file_path.lower():
            mime_type = "text/html" # oops, this is wrong
        elif jpg_type in file_path.lower():
            mime_type = "image/jpeg"
        elif jpeg_type in file_path.lower():
            mime_type = "image/jpeg"
        elif png_type in file_path.lower():
            mime_type = "image/png"
        elif txt_type in file_path.lower():
            mime_type = "text/plain"
        elif css_type in file_path.lower():
            mime_type = "text/css"
        elif js_type in file_path.lower():
...
def handle_http_get(req, conn):
    # Generate a response
    global count
    if '/whisper/feed/' in req.path:
        select = re.sub("/whisper/feed/","",req.path)
        selected_topic_name = select.split("?", 1)[0]
        selected_version = int(select.split("version=",1)[1])
        selected_topic = is_in_list(selected_topic_name)
        if selected_topic is None:
            resp = Response("404 NOT FOUND", "text/plain", "Topic does not exist.")
        elif not isinstance(selected_version, int):
            resp = Response("400 BAD REQUEST", "text/plain", "Request is malformed.")
        else:       
            with selected_topic.lock:
                while selected_topic.version_num < selected_version:
                    selected_topic.lock.wait()
            msg_feed = f'{selected_topic.version_num}\n'
            if selected_topic.messages:
                for message in selected_topic.messages:
                    msg_feed+= f'- {message}\n'
            resp = Response("200 OK", "text/plain", msg_feed)    

    elif '/whisper/topics?' in req.path:
        v_num = int(req.path.split('=',1)[1])
        with updates:
            while topic_list_version < v_num:
                updates.wait()
            
        resp = handle_get_topics_list()
...
def handle_get_topics_list():
    with updates:
        global topic_list_version
        msg = f"{topic_list_version}\n"
        for topic in topics:
            msg += f"{topic.msg_count} {topic.like_count} {topic}\n"
        return Response("200 OK", "text/plain", msg)
...
def handle_http_post(req):
    ...
    if '/whisper/like/' in req.path: 
        liked_topic_name = re.sub("/whisper/like/","", req.path)
        liked_topic = is_in_list(liked_topic_name)
        if liked_topic is None:
            resp = Response("404 NOT FOUND", "text/plain", "Topic does not exist.")
        else:
            add_like(liked_topic)
            resp = Response("200 OK","text/plain", "success")
    elif req.path == "/whisper/messages":
        parameters = req.body.split("\n")
        if len(parameters) < 2:
           resp = Response("400 BAD REQUEST", "text/plain", "Payload must be two lines.")
           return resp

        if parameters[0].startswith("tags..."):
            hashtags = re.sub("tags...","", parameters[0]).split()
            current_message = re.sub("message...","", parameters[1])
            if len(current_message) == 0:
                resp =  Response("200 OK","text/plain", "Empty messages are ignored.")
                return resp
            if not hashtags:
                hashtags = ["miscellaneous"]
            for item in hashtags:
                if is_in_list(item) == None:
                    current_topic = Topic(item)
                    with updates:
                        topics.append(current_topic)
                        topic_list_version +=1
                        updates.notify_all()
                else:
                    current_topic = is_in_list(item)
                add_msg(current_topic, current_message)
            resp = Response("200 OK","text/plain", "success")

    return resp         


def is_in_list(topic_name):
    global topics
    for topic in topics:
...
def add_msg(topic, new_message):
    global topic_list_version     
    with topic.lock:
        topic.msg_count+=1
        topic.version_num +=1
        topic.messages.append(new_message)
        topic.lock.notify_all()
    with updates:
        topic_list_version +=1
        updates.notify_all()
